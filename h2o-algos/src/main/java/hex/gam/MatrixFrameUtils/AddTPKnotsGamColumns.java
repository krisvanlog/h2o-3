...
import static hex.gam.GamSplines.ThinPlatePolynomialBasisUtils.extractColNames;
import static hex.gam.GamSplines.ThinPlatePolynomialBasisUtils.genThinPlateNameStart;
...
public class AddTPKnotsGamColumns {
  ...
  public void addTPGamCols() {
    ...
    for (int index = 0; index < _numTPCols; index++) { // generate smoothers/splines
      ...
      generateGamColumn[index] = new ApplyTPRegressionSmootherWithKnots(predictVec, _parms, index, _knots[offsetIndex],
              index, _zCS[index], _z[index], _polyBasisList[index]);
    }
